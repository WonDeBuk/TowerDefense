...
Healbot::Healbot() {
	...
	EnemyHitbox = { EnemyCurrentPosition.x - 48.0f, EnemyCurrentPosition.y - 0.9f * 96.0f, 96.0f, 96.0f };
...
void Healbot::HealTarget() {
	...
	if (gm.GetCurrentEnemyAmount() <= 1) {
		VisualManager::GetInstance().AddVisual(VisualType::ENEMY_BIND, EnemyBindVisual::EnemyBindVisualTemplateBuildAndGet("ui/HealEffect.png", 7, { 78.0f, 156.0f }, { 156.0f, 156.0f }, EnemyID));
		//VisualManager::GetInstance().AddImageVisual(7, "ui/HealEffect.png", { EnemyCurrentPosition.x - 78.0f, EnemyCurrentPosition.y - 120.0f }, { 156.0f, 156.0f });
		OnHeal(BASE_HEAL / 3);
	}
	else {
		...
		if (LeastHealth) {
			VisualManager::GetInstance().AddVisual(VisualType::ENEMY_BIND, EnemyBindVisual::EnemyBindVisualTemplateBuildAndGet("ui/HealEffect.png", 7, { 78.0f, 156.0f }, { 156.0f, 156.0f }, LeastHealth->GetEnemyID()));
			//VisualManager::GetInstance().AddImageVisual(7, "ui/HealEffect.png", { LeastHealth->GetEnemyCurrentPosition().x - 78.0f, LeastHealth->GetEnemyCurrentPosition().y - 120.0f }, { 156.0f, 156.0f });
